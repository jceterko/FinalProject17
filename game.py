...
print(f"""Here is {s.name}. He is a {s.species}. You have anything you would like to say {s.name}?
{s.name}: {s.famous_quote}""")
print(f"""Next up is {d.name}. He is also a {d.species}. And here he is!
{d.name}: {d.famous_quote}""")
print(f"""Lastly, the beloved {cas.name}. He is a {cas.species}. OoOoOoOoO Ahhhhhhh. Anything you wanna say {cas.name}?
{cas.name}: {cas.famous_quote}""")
...
print("""You are now in an abandoned barn in the middle of Kansas and you notice it is getting extremely cold inside depsite the fact that is summer.
Then, suddenly, a figure appears in the darkness...""")
...
qa = [com0 , com1, com2, com3, com4, com5, com6, com7, com8]
...
response = input('You are now looking around the barn and see a old red door that is covered in weird goo. Do you want to go in? ').lower()
if response == 'yes':
...
print(f'I am not gonna lie, I am quite flabbergasted. I was not expecting you to defeat {lil.name} so easily. Buttttt you are not done yet, now you only have to get through one more door, specifically the blue door that looks like it is begging someone to smash into it.')
# leaving stage 3

respone = input(f'Now {c}, do you want to go through the door? ').lower()
...
print(f'OMG OMG OMG OMG YOU ACTUALLY DEFEATED {luc.name} HOLY SHIZ YOU DID IT!')
print(f'Ok now all you have to do is open that dirty looking box')

response = input(f'Do you want to open the box {c}? ').lower()
if response == 'yes':
...
# using if else statement to open the box

print(f'I\'m so proud of you {c}, you have finished this game, now go and finish the rest of your midterms! yay')
# end of the game
